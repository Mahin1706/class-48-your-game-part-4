...
var player;
var img, ballImg, runLeft, runRight, wallImg,fireImg , batImg;
var lava1, bumpyWall;
...
function draw() {
  ...
  if (gameState === 'playState'){
    ...
    if(player.isTouching(lavaGroup)||player.isTouching(batGroup)){
      player.x = 40;
      player.y = 330;
      deaths = deaths + 1;
...
function spawnBat(){
  if(frameCount%50===0){
    var bat =createSprite(40,40,40,40);
    bat.scale=0.1;
    bat.x=Math.round(random (40,650));
    bat.y=Math.round(random (40,250));
    bat.addImage(batImg);
    bat.velocityX=Math.round(random(-2,4));
    bat.velocityY=Math.round(random(-2,4));
    bat.lifetime=200;
